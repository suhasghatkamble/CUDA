//Assignment : Modify the 2-D array addition code to display globally unique thread number for every thread.
...
//   int final_threadId = ;
  ...
  if (row < M && col < N) {
    ...
    printf("Total global threadid = %d \n", row*blockDim+col);
